-- ADR-0074 / #888 — subject-tracking reframe generation job role.
...
alter table public.generation_jobs
  ...
  check (role in (
    ...
    'speech_enhance', 'reframe'
  ));

comment on constraint generation_jobs_role_check on public.generation_jobs is
  'Generator roles plus extract, index, music, Voice Studio, speech enhance, and reframe (ADR-0074).';
